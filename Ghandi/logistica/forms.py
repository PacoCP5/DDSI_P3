...
class HacerPedidoForm(forms.Form):
    idpedido = forms.IntegerField(label="idPedido", required=False,widget=forms.TextInput(attrs={'placeholder': "idPedido (Opcional)"}))
    ...
    pagado = forms.CharField(label="Pagado",required=False, max_length=1, widget=forms.TextInput(attrs={'placeholder': "Pagado (S/N) (Opcional)"}))
